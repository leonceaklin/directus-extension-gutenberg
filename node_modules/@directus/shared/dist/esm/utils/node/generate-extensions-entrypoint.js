...
export function generateExtensionsEntrypoint(extensions) {
    const appOrHybridExtensions = extensions.filter((extension) => isIn(extension.type, [...APP_EXTENSION_TYPES, ...HYBRID_EXTENSION_TYPES]));
    ...
    const appOrHybridExtensionImports = [...APP_EXTENSION_TYPES, ...HYBRID_EXTENSION_TYPES].flatMap((type) => appOrHybridExtensions
    ...
    const bundleExtensionImports = bundleExtensions.map((extension, i) => `import {${[...APP_EXTENSION_TYPES, ...HYBRID_EXTENSION_TYPES]
    ...
    const extensionExports = [...APP_EXTENSION_TYPES, ...HYBRID_EXTENSION_TYPES].map((type) => `export const ${pluralize(type)} = [${appOrHybridExtensions
        .filter((extension) => extension.type === type)
        .map((_, i) => `${type}${i}`)
