...
import { getFreeformContentHandlerName, getDefaultBlockName, __unstableSerializeAndClean } from '@wordpress/blocks';
...
export const isSavingPost = createRegistrySelector(select => state => {
  const postType = getCurrentPostType(state);
  const postId = getCurrentPostId(state);
  return select(coreStore).isSavingEntityRecord('postType', postType, postId);
});
/**
...
export function __experimentalGetTemplateInfo(state, template) {
  ...
  const templateIcon = ((_experimentalGetDefa = __experimentalGetDefaultTemplatePartAreas(state).find(item => area === item.area)) === null || _experimentalGetDefa === void 0 ? void 0 : _experimentalGetDefa.icon) || layout;
  return {
