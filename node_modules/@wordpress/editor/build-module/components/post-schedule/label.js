...
export function getFullPostScheduleLabel(dateAttribute) {
  ...
  const formattedDate = dateI18n( // translators: If using a space between 'g:i' and 'a', use a non-breaking sapce.
...
export function getPostScheduleLabel(dateAttribute) {
  ...
  if (isSameDay(date, now)) {
    ...
    __('Today at %s'), // translators: If using a space between 'g:i' and 'a', use a non-breaking sapce.
  ...
  if (isSameDay(date, tomorrow)) {
    return sprintf( // translators: %s: Time of day the post is scheduled for.
    __('Tomorrow at %s'), // translators: If using a space between 'g:i' and 'a', use a non-breaking sapce.
    dateI18n(_x('g:i\xa0a', 'post schedule time format'), date));
  }

  if (date.getFullYear() === now.getFullYear()) {
    return dateI18n( // translators: If using a space between 'g:i' and 'a', use a non-breaking sapce.
    _x('F j g:i\xa0a', 'post schedule date format without year'), date);
  }

  return dateI18n( // translators: Use a non-breaking space between 'g:i' and 'a' if appropriate.
  _x('F j, Y g:i\xa0a', 'post schedule full date format'), date);
}
