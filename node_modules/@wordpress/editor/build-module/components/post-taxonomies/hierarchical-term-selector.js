...
import { Button, CheckboxControl, TextControl, TreeSelect, withFilters } from '@wordpress/components';
...
export function HierarchicalTermSelector(_ref) {
  let {
    slug
  } = _ref;
  ...
  const {
    ...
    return {
      hasCreateAction: _taxonomy ? get(getCurrentPost(), ['_links', 'wp:action-create-' + _taxonomy.rest_base], false) : false,
      hasAssignAction: _taxonomy ? get(getCurrentPost(), ['_links', 'wp:action-assign-' + _taxonomy.rest_base], false) : false,
  ...
  const availableTermsTree = useMemo(() => sortBySelected(buildTermsTree(availableTerms), terms), // Remove `terms` from the dependency list to avoid reordering every time
  ...
  const onAddTerm = async event => {
    ...
    _x('%s added', 'term'), get(taxonomy, ['labels', 'singular_name'], slug === 'category' ? __('Category') : __('Term')));
  ...
  return createElement(Fragment, null, showFilter && createElement(TextControl, {
  ...
  }, renderTerms('' !== filterValue ? filteredTermsTree : availableTermsTree)), !loading && hasCreateAction && createElement(Button, {
